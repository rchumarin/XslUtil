...
Из Java кода:
1) запускается saxon парсер, который обрабатывает xml и xsl файлы. Преобразование xml-файлов осуществляется по алгоритму, заданному в xsl-файлах. 
Поскольку записи-вложения полностью не преобразуются к нужному виду (алгоритм формирования подписи двух видов см. ниже), то поэтому конечное преобразование осуществляется непосредственно с самом java-коде, а именно:
