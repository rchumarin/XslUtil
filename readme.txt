...
Суть проекта: имеются платежные документы (xml - файлы) содержащие различную платежную информацию, электронную подпись и вложения. 

Из Java кода:
1) запускается saxon парсер, который обрабатывает xml и xsl файлы, создает Строку с данными по алгоритму формирования подписи версии 0 или 1.  Предварительный алгоритм преобразования заложен в xsl-файлах.
2) осуществляется чтение созданной строки, некоторые записи (вложения) преобразуются к определенному виду (см. ниже) и кодируются в бинарный вид (Base64).
